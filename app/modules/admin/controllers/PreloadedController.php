...
class PreloadedController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{            
			$model->attributes=$_POST['Preloaded'];
			
            if ($model->save()) {
                $model->imageUploader = CUploadedFile::getInstanceByName('Preloaded[image]');
	...
	{        
		...
		{            
            $model->imageUploader = CUploadedFile::getInstanceByName('Preloaded[image]');
			$model->attributes=$_POST['Preloaded'];
            if ($_POST['Preloaded']['deleteImage']) {
                $model->deleteImage();
                $model->image = NULL;                
            }
	...
	{
		$model=new Preloaded('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Preloaded']))
			$model->attributes=$_GET['Preloaded'];
	...
	{
		$model=Preloaded::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='preloaded-form')
		{
