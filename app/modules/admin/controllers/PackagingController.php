...
class PackagingController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{            
			$model->attributes=$_POST['Packaging'];
			
            if ($model->save()) {
                $model->imageUploader = CUploadedFile::getInstanceByName('Packaging[image]');
                $model->uploadImage();
                $model->imageUploader = CUploadedFile::getInstanceByName('Packaging[image2]');
                $model->uploadImage(true); //alternative image
	...
	{        
		...
		{                                
			$model->attributes=$_POST['Packaging'];
            if ($_POST['Packaging']['deleteImage']) {
                $model->deleteImage();
                $model->image = NULL;                
            }
            if ($_POST['Packaging']['deleteImage2']) {
                $model->deleteImage2();
                $model->image2 = NULL;                
            }
			if($model->save()) {
                ...
                if (!$model->hasErrors()) $this->redirect(array('view','id'=>$model->id));            
            }
		}
	...
	{
		$model=new Packaging('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Packaging']))
			$model->attributes=$_GET['Packaging'];
	...
	{
		$model=Packaging::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='packaging-form')
		{
