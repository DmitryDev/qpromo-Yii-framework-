...
class CategoriesTree extends CWidget {
    ...
    public function init() {
        ...
        if ($this->editable) {
            // Publishing and registering JavaScript
            // Uncomment the line below if need to add javascript dinamycs
            //$cs->registerCoreScript("jquery");
            //$cs->registerScriptFile( Yii::app()->assetManager->publish($assetsDir.'/CategoriesTree.js'), CClientScript::POS_END );
        }
        ...
        elseif ($this->model instanceof Category) {
    ...
    private function _echoControls($category) {
        ...
        $view_link  = CHtml::link($view_img,    array('view',   'id'=>$category->id));
        $edit_link  = CHtml::link($edit_img,    array('update', 'id'=>$category->id));
        $delete_link= CHtml::link($delete_img,  array('delete', 'id'=>$category->id), array(
