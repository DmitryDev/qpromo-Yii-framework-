...
class Environment
{
    ...
    private function _detectEnv()
    {
        ...
        if (!is_null($this->_env)) return $this->_env;

        foreach($this->getEnvs() as $domain => $env) {
            if (preg_match($this->_getDomainPattern($domain), $this->_host)) return $env;            
        }
    }
    ...
    private function _getConfigParams($configType = null)
    {
        ...
        if (is_null($configType)) return require($this->_configDir . $appPrefix . '.php');
        return require($this->_configDir . $appPrefix . '.' . $configType . '.php');
    }
    ...
    private function _mergeArray($a,$b)
    {
        foreach($b as $k => $v)
        {
            if(is_integer($k)) isset($a[$k]) ? $a[] = $v : $a[$k] = $v;
