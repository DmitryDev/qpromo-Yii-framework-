<?php include_once('_header.php'); ?>

<div id="content">
<div class="text_intro">Makers of the coolest high-tech promotional items!</div>

<div id="slider" style="height: 100%">
    <div class="slider_inner" style="height: 100%">
        <div class="flexslider">
            <ul class="slides">
                <li>
                    <img src="img/slide_test.png" />
                </li>
                <li>
                    <img src="img/slide_test.png" />
                </li>
                <li>
                    <img src="img/slide_test.png" />
                </li>
                <li>
                    <img src="img/slide_test.png" />
                </li>
            </ul>
        </div>
    </div>
</div><!-- #slider -->

<div class="section top_shadow">
    <div class="section_inner bottom_shadow">
        <div class="subnav">
            <ul class="nav2">
                <li><a href="#" class="active">USB</a></li>
                <li><a href="#">Mobile</a></li>
                <li><a href="#">Tablet</a></li>
                <li><a href="#">Audio</a></li>
                <li><a href="#">Other</a></li>
            </ul>
            <div class="sep"></div>
            <ul class="nav3">
                <li><a href="#" class="active">Popular</a></li>
                <li><a href="#">New Releases</a></li>
            </ul>
        </div>
        <ul class="thumb_list">
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
            <li class="list_item">
                <a href="#" class="thumb"><img src="img/thumb.jpg"></a>
                <div class="title">Alpine Pro</div>
                <div class="desc">1GB - 16GB</div>
                <ul class="colors">
                    <li><span class="color" style="background: black"></span></li>
                    <li><span class="color" style="background: blue"></span></li>
                    <li><span class="color" style="background: white"></span></li>
                    <li><span class="color" style="background: gray"></span></li>
                    <li><span class="color" style="background: red"></span></li>
                    <li><span class="color" style="background: yellow"></span></li>
                </ul>
            </li>
        </ul>
    </div>
</div><!-- .section -->

<div id="home_events">
    <div class="title">Upcoming Events</div>
    <ul>
        <li class="event">
            <div class="event_title">January</div>
            <div class="event_date">15-18</div>
            <div class="event_desc">Proforma Regional</div>
            <div class="event_place">Las Vegas, NV</div>
        </li>
        <li class="event">
            <div class="event_title">January</div>
            <div class="event_date">15-18</div>
            <div class="event_desc">Proforma Regional</div>
            <div class="event_place">Las Vegas, NV</div>
        </li>
        <li class="event">
            <div class="event_title">January</div>
            <div class="event_date">15-18</div>
            <div class="event_desc">Proforma Regional</div>
            <div class="event_place">Las Vegas, NV</div>
        </li>
    </ul>
</div><!-- #home_events -->

<div id="q_club" class="top_shadow">
    <div class="title">Q/Club</div>
    <div class="q_club_inner">
        <div class="q_club_item">
            <div class="q_club_thumb">
                <img src="img/q_club_demo.jpg" alt="" title="" />
            </div>
            <div class="q_club_desc">
                Title of the blog post / promotional feature will go here and possibly here if the title is very long like in this case.
            </div>
        </div>
        <div class="q_club_item">
            <div class="q_club_thumb">
                <img src="img/q_club_demo.jpg" alt="" title="" />
            </div>
            <div class="q_club_desc">
                Title of the blog post / promotional feature will go here and possibly here if the title is very long like in this case.
            </div>
        </div>
    </div>
</div><!-- #q_club -->

</div><!-- #content -->

<?php include_once('_footer.php'); ?>